...
namespace cct::gfx::rhi
{
	...
	public:
		...
		const std::shared_ptr<DescriptorSetLayout>& GetLayout() const override { return m_layout; }

		// DX12-specific: Get the CBV/SRV/UAV descriptor range
		const dx12::DescriptorRange& GetDescriptorRange() const noexcept { return m_gpuRange; }
		// DX12-specific: Get the sampler descriptor range
		const dx12::DescriptorRange& GetSamplerRange() const noexcept { return m_samplerRange; }
		bool HasSamplers() const noexcept { return m_samplerRange.baseHandle.IsValid(); }
