...
namespace cct::gfx::rhi
{
	...
	public:
		...
		const std::vector<D3D12_DESCRIPTOR_RANGE1>& GetDx12Ranges() const noexcept { return m_dx12Ranges; }

		// DX12-specific: Get sampler descriptor ranges (separate heap)
		const std::vector<D3D12_DESCRIPTOR_RANGE1>& GetDx12SamplerRanges() const noexcept { return m_dx12SamplerRanges; }

		// Get combined shader visibility from all bindings
		D3D12_SHADER_VISIBILITY GetShaderVisibility() const noexcept { return m_shaderVisibility; }

		// Get total number of descriptors across all bindings
		UINT GetTotalDescriptorCount() const noexcept { return m_totalDescriptorCount; }

		// Check if this layout contains samplers
		bool HasSamplers() const noexcept { return !m_dx12SamplerRanges.empty(); }
