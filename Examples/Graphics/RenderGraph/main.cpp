...
int main()
{
    try
    {
        ...
        rhi::RGTextureHandle albedo = rg.CreateTexture({
            W, H, PixelFormat::RGBA8_SRGB, false, "GBufferAlbedo" });

        rhi::RGTextureHandle depth = rg.CreateTexture({
            W, H, PixelFormat::D32f, true, "GBufferDepth" });

        // ------------------------------------------------------------------
        // CulledPass: writes to a texture that nobody reads.
        // After Compile() it will be marked as culled.
        // ------------------------------------------------------------------
        rhi::RGTextureHandle unusedTex = rg.CreateTexture({
            W, H, PixelFormat::RGBA8_SRGB, false, "UnusedOutput" });

        rg.AddGraphicsPass("CulledPass",
            [&](rhi::RenderGraphBuilder& b) { unusedTex = b.Write(unusedTex); },
            [](rhi::RenderGraphContext&) {});
