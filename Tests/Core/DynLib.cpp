...
namespace CCT_ANONYMOUS_NAMESPACE
{
	SCENARIO("DynLib - Invoke")
	{
		GIVEN("A DynLib loaded with the dummy library")
		{
			...
			WHEN("Dummy() is invoked")
			{
				...
				THEN("The function handle is valid")
				{
			...
			WHEN("DummyInt() is invoked")
			{
				THEN("It returns 42")
				{
					CHECK(lib.Invoke<int>("DummyInt") == 42);
				}
			}
			...
			WHEN("Increment(5) is invoked")
			{
				THEN("It returns 6")
				{
					CHECK(lib.Invoke<int>("Increment", 5) == 6);
				}
			}
			...
			WHEN("A non-existing function is invoked")
			{
				...
				{
					CHECK_THROWS_AS(lib.Invoke<int>("NotExisting", 5), std::runtime_error);
				}
			}
	...
	SCENARIO("DynLib - Loading")
	{
		GIVEN("An empty DynLib")
		{
			...
			THEN("Unload on an empty library returns false")
			{
