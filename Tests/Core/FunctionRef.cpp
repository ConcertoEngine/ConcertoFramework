...
namespace CCT_ANONYMOUS_NAMESPACE
{
	...
	SCENARIO("FunctionRef")
	{
		GIVEN("An empty FunctionRef<void()>")
		{
			...
			{
				REQUIRE(func.operator bool() == false);
			}
			...
			WHEN("A lambda is assigned")
			{
				...
				{
					REQUIRE(func.operator bool() == true);
				}
			}
		}
		...
		GIVEN("A FunctionRef<void()> holding a capturing lambda")
		{
			...
			{
				func();
				THEN("The captured value is set to 42")
				{
					REQUIRE(value == 42);
				}
			}
		...
		{
			auto lambda = [](std::size_t& value, const std::string& str)
			{ value = str.size(); };
			...
			WHEN("Called with a string of length 5")
			{
				...
				func(value, str);
				THEN("value equals the string size")
				{
					REQUIRE(value == str.size());
				}
			}
		}
	}
